...
export class PDFExternalLinkPostProcessor extends PDFReaderComponent implements HoverParent {
	...
	onload() {
		if (
			this.settings.popoverPreviewOnExternalLinkHover &&
	...
	static registerEvents(plugin: PDFReader, child: PDFViewerChild, annot: AnnotationElement) {
		if (annot.data.subtype === "Link" && annot.data.url) {
			return child.component?.addChild(
				new PDFExternalLinkPostProcessor(plugin, child, annot),
			);
		}
		return null;
	}
