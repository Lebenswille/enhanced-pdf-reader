...
abstract class PDFLinkLikePostProcessor implements HoverParent {
	...
	protected constructor(plugin: PDFReader, child: PDFViewerChild, targetEl: HTMLElement) {
		this.plugin = plugin;
		this.app = plugin.app;
		this.lib = plugin.lib;
		this.child = child;
	...
	private registerClickToOpenInNewLeaf() {
		...
		plugin.registerDomEvent(
			...
			{ capture: true },
		); // capture to ensure it's called before the default click handler
	}
		...
		plugin.registerDomEvent(targetEl, "mouseover", async (event) => {
			...
			try {
				...
				if (e.name === "UnknownErrorException") {
					return console.warn(
						`${this.plugin.manifest.name}: The destination was not found in this document.`,
					);
				}
				throw e;
			}
...
export class PDFInternalLinkPostProcessor extends PDFDestinationHolderPostProcessor {
	...
	async getLinkText(evt: MouseEvent) {
		if (
			this.plugin.settings.actionOnCitationHover === "google-scholar-popover" &&
	...
	shouldRecordHistory() {
		return this.plugin.settings.recordPDFInternalLinkHistory && !this.child.opts.isEmbed;
	}
...
export class PDFOutlineItemPostProcessor extends PDFDestinationHolderPostProcessor {
	...
	static registerEvents(plugin: PDFReader, child: PDFViewerChild, item: PDFOutlineTreeNode) {
	...
	shouldRecordHistory() {
		return this.plugin.settings.recordHistoryOnOutlineClick && !this.child.opts.isEmbed;
	}
...
export class PDFThumbnailItemPostProcessor extends PDFLinkLikePostProcessor {
	...
	async getLinkText(evt: MouseEvent) {
		const anchorEl =
			...
			evt.target.closest('.pdf-thumbnail-view > a[href^="#page="]');
	...
	shouldRecordHistory() {
		return this.plugin.settings.recordHistoryOnThumbnailClick && !this.child.opts.isEmbed;
	}
