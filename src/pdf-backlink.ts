...
import {
	MutationObservingChild,
	getSubpathWithoutHash,
	isMouseEventExternal,
	isTargetHTMLElement,
} from "utils";
...
export class BacklinkPanePDFManager extends PDFReaderComponent {
	...
	constructor(plugin: PDFReader, renderer: BacklinkRenderer, file: TFile) {
		super(plugin);
	...
	onload() {
		...
		this.registerDomEvent(this.renderer.backlinkDom.el, "mouseover", (evt) => {
			this.processBacklinkVisualizerDomForEvent(
				evt,
				(backlinkItemEl, visDoms, cache, viewer) => {
					...
					if (cache.page && cache.annotation) {
						...
						viewer.then((child) => {
							...
							const annot =
								pageView.annotationLayer?.annotationLayer.getAnnotation(annotId);
							if (annot) {
								rectEl = this.lib.highlight.viewer.placeRectInPage(
									annot.data.rect,
									pageView,
								);
	...
	findBacklinkItemEl(cache: PDFBacklinkCache): HTMLElement | null {
		...
		const isBetterSearchViewsEnabled = !!fileDom.childrenEl.querySelector(
		...
		if (!isBetterSearchViewsEnabled) {
			...
			const itemDom = itemDoms.find((itemDom) => {
				...
				if ("position" in refCache) {
					return (
						itemDom.start <= refCache.position.start.offset &&
						refCache.position.end.offset <= itemDom.end
					);
				}
			...
			for (const [start, end] of fileDom.result.content) {
				const sec = cache.sections.find(
					(sec) => sec.position.start.offset <= start && end <= sec.position.end.offset,
				);
				if (sec) {
					sectionsContainingBacklinks.add(sec);
					if (
						start === refCache.position.start.offset &&
						refCache.position.end.offset === end
					) {
						break;
					}
				}
			...
			return (
				fileDom?.childrenEl.querySelectorAll<HTMLElement>(".search-result-file-match")[
					index
				] ?? null
			);
		}
	...
	processBacklinkVisualizerDomForEvent(
		evt: MouseEvent,
		callback: (
			backlinkItemEl: HTMLElement,
			visualizerEls: Set<HTMLElement>,
				...
				if (viewer.visualizer) {
					...
					for (const cache of caches) {
						...
						if (backlinkItemEl?.contains(targetEl)) {
							const cacheToDoms = viewer.visualizer.domManager.getCacheToDomsMap(
								cache.page,
							);
...
export class BacklinkPanePDFPageTracker extends PDFReaderComponent {
	matchCountObserver: MutationObservingChild;

	constructor(
		plugin: PDFReader,
		public renderer: BacklinkRenderer,
		public file: TFile,
	) {
		super(plugin);
		this.matchCountObserver = new MutationObservingChild(
			...
			() => {
				...
				if (!this.renderer.collapseAll) {
					...
					// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/121
					for (const resultEl of this.renderer.backlinkDom.el.querySelectorAll(
						".tree-item.search-result:not(:has( .search-result-file-match))",
					)) {
					...
					for (const resultEl of this.renderer.backlinkDom.el.querySelectorAll(
						...
						const collapseIconEl = resultEl.querySelector<HTMLElement>(
							":scope>.tree-item-self.search-result-file-title>.collapse-icon",
						);
						if (collapseIconEl) {
		...
		if (leaf) {
			...
			if (this.lib.isPDFView(view)) {
				view.viewer.then((child) => {
					...
					this.lib.registerPDFEvent(
						"pagechanging",
						child.pdfViewer.eventBus,
						this,
						(data) => {
							const page =
								typeof data.pageNumber === "number"
									? (data.pageNumber as number)
									: child.pdfViewer.pdfViewer?.currentPageNumber;
							if (page)
								this.renderer.backlinkDom.filter = (file, linkCache) =>
									this.filter(page, linkCache);
	...
	updateBacklinkCountEl(format?: (num: number) => string) {
		const num = Array.from(
			this.renderer.backlinkDom.el.querySelectorAll<HTMLElement>(
				"div.search-result-file-title span.tree-item-flair",
			),
		)
			.map((el) => +el.getText())
			.reduce((a, b) => a + b, 0);
