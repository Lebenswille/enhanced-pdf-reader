...
export class VimHintMode extends VimBindingsMode {
	...
	hintPage(pageNumber: number) {
		...
		if (numHints === 0) {
			new Notice(
				`${this.plugin.manifest.name} (Vim mode): No hintable element found on this page`,
			);
			this.exit();
		...
		hintableEls.forEach((hintableEl) => {
			...
			if (VimHintMode.isLink(hintableEl)) {
				...
				if (
					prevLinkEl &&
					prevLinkEl.href === thisLinkEl.href &&
					isCloseTo(prevLinkEl, thisLinkEl)
				) {
					return;
				}
...
const isCloseTo = (prevLinkEl: HTMLElement, thisLinkEl: HTMLElement) => {
	...
	return (
		// For horizontal text
		(Math.abs((thisRect.top + thisRect.bottom) / 2 - (prevRect.top + prevRect.bottom) / 2) <
			yThreshold &&
			(Math.abs(thisRect.left - prevRect.right) < xThreshold ||
				Math.abs(thisRect.right - prevRect.left) < xThreshold)) ||
		// For vertical text
		(Math.abs((thisRect.left + thisRect.right) / 2 - (prevRect.left + prevRect.right) / 2) <
			yThreshold &&
