...
export class VimCommandLineMode extends VimBindingsMode {
	...
	constructor(vim: VimBindings) {
		...
		this.suggest = new ExcmdSuggest(this).onSelect(({ item: { minNargs: nargs } }) =>
	...
	async executeCommand(
		cmd: string,
		options: Partial<{ error: ErrorReportMethod[]; history: boolean }> = {
			error: ["notice", "console.error"],
			history: true,
		},
	) {
		...
		if (cmd.startsWith("!")) {
			if (!Platform.isDesktopApp) {
				this.reportError(
					`${this.plugin.manifest.name} (Vim mode): Shell command is not supported on mobile`,
					options.error!,
				);
				return;
			}
			...
			return new Promise<string>((resolve, reject) =>
				exec(cmd.slice(1), { env }, (err, stdout, stderr) => {
					if (err) reject(err);
		...
		if (excmd) {
			if (excmd.minNargs && args.length < excmd.minNargs) {
				this.reportError(
					`${this.plugin.manifest.name} (Vim mode): Expected ${excmd.minNargs} or more arguments for command "${excmd.id}" but got ${args.length}`,
					options.error!,
				);
				return;
			}
		...
		this.reportError(
			`${this.plugin.manifest.name} (Vim mode): Unknown command "${cmd}"`,
			options.error!,
		);
	}

	findCommand(excmdName: string) {
		return this.excmds.find(
			(excmd) => (excmd.pattern && excmd.pattern.test(excmdName)) || excmd.id === excmdName,
		);
	}
	...
	submitCommand() {
		...
		if (cmd) {
			...
			try {
				...
				new Notice(
					`${this.plugin.manifest.name} (Vim mode): Error occurred while executing the command : ${err}`,
				);
				console.error(err);
			}
		}
	...
	runScript(script: string) {
		...
		cmds.forEach((cmd) =>
			this.executeCommand(cmd, { error: ["console.warn"], history: false }),
		);
	}
...
class ExcmdSuggest extends FuzzyInputSuggest<ExCommand> {
	...
	constructor(commandLineMode: VimCommandLineMode) {
		...
		const escapeHandler = this.scope.keys.find(
			(key) => key.key === "Escape" && key.modifiers === "",
		);
		if (escapeHandler) this.scope.unregister(escapeHandler);

		const onArrowDown = this.scope.keys.find(
			(key) => key.key === "ArrowDown" && key.modifiers === "",
		);
		const onArrowUp = this.scope.keys.find(
			(key) => key.key === "ArrowUp" && key.modifiers === "",
		);
		if (onArrowDown) {
