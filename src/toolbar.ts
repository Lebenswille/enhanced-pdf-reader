...
export class PDFReaderToolbar extends PDFReaderComponent {
	static elInstanceMap = new Map<HTMLElement, PDFReaderToolbar>();

	toolbar: PDFToolbar;
	child: PDFViewerChild;

	constructor(plugin: PDFReader, toolbar: PDFToolbar, child: PDFViewerChild) {
		super(plugin);
	...
	makeDropdownInToolbarHoverable() {
		...
		toolbar.toolbarLeftEl
			.querySelectorAll<HTMLElement>("div.clickable-icon")
			.forEach((buttonEl) => {
	...
	replaceDisplayOptionsDropdown() {
		...
		toolbar.zoomInEl.after(
			createDiv("clickable-icon", (dropdownEl) => {
	...
	addZoomLevelInputEl() {
		...
		toolbar.zoomOutEl.after(
			createEl("input", "pdf-zoom-level-input", (inputEl) => {
				...
				inputEl.addEventListener("change", () => {
					...
					const clamped = Math.min(
						Math.max(value, window.pdfjsViewer.MIN_SCALE),
						window.pdfjsViewer.MAX_SCALE,
					);
				...
				inputEl.doc.win.setTimeout(() => {
					inputEl.after(
						createSpan({ cls: "pdf-zoom-level-percent", text: "%" }, (spanEl) => {
