...
export class BibliographyManager extends PDFReaderComponent {
	...
	isEnabled() {
		...
		return (
			this.settings.actionOnCitationHover !== "none" &&
	...
	on(
		name: "extracted",
		callback: (destId: string, bibText: string) => any,
		ctx?: any,
	): ReturnType<Events["on"]>;
	on(
		name: "parsed",
		callback: (destId: string, parsedBib: string) => any,
		ctx?: any,
	): ReturnType<Events["on"]>;
...
class BibliographyTextExtractor {
	...
	constructor(plugin: PDFReader, doc: PDFDocumentProxy) {
		this.plugin = plugin;
		this.doc = doc;
	...
	async extract() {
		...
		for (const destId in dests) {
			if (this.plugin.lib.isCitationId(destId)) {
				...
				promises.push(
					this.extractBibTextForDest(destArray).then((bibInfo) => {
						if (bibInfo) {
	...
	async getTextContentItemsFromPageRef(pageRef: PDFJsDestArray[0]) {
		...
		return (
			this.pageRefToTextContentItemsPromise[refStr] ??
...
export class BibliographyDom extends PDFReaderComponent {
	...
	async render() {
		...
		this.containerEl.createDiv("button-container", (el) => {
			new ButtonComponent(el).setButtonText("Google Scholar").onClick(() => {
				const url = this.bib.getGoogleScholarSearchUrlFromDest(this.destId);
				if (!url) {
					new Notice(
						`${this.plugin.manifest.name}: ${this.bib.initialized ? "No bibliography found" : "Still loading the bibliography information. Please try again later."}`,
					);
					return;
				}
				window.open(url);
