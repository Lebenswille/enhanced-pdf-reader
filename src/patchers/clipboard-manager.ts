...
export const patchClipboardManager = (plugin: PDFReader) => {
	...
	plugin.register(
		around(clipboardManager.constructor.prototype, {
			/**
			...
			handleDragOver(old) {
				return function (this: ClipboardManager, evt: DragEvent): void {
					...
					else // if (draggable.type === 'annotation-link')
					{
			...
			handleDrop(old) {
				return function (this: ClipboardManager, evt: DragEvent): boolean | undefined {
					...
					if (
						this.info instanceof MarkdownView &&
...
function setDragEffect(evt: DragEvent, dropEffect: DropEffect) {
	...
	if (dropEffect === "none") return (evt.dataTransfer.dropEffect = dropEffect);
