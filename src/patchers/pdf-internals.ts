...
import { PDFReaderToolbar } from "toolbar";
import { BibliographyManager } from "bib";
import {
	camelCaseToKebabCase,
	getCharactersWithBoundingBoxesInPDFCoords,
	getTextLayerInfo,
	hookInternalLinkMouseEventHandlers,
	isEmbed,
	isModifierName,
	isNonEmbedLike,
	selectDoubleClickedWord,
	selectTrippleClickedTextLayerNode,
...
export const patchPDFInternals = async (
	plugin: PDFReader,
...
function onPDFInternalsPatchSuccess(plugin: PDFReader) {
	const { lib } = plugin;
	// For the detail of `plugin.subpathWhenPatched`, see its docstring.
	lib.workspace.iteratePDFViews((view) =>
		reloadPDFViewerComponent(view.viewer, view.file, plugin.subpathWhenPatched),
	);
	...
	// See https://github.com/RyotaUshio/obsidian-pdf-reader/issues/322
	lib.workspace.iteratePDFEmbeds((embed) =>
		reloadPDFViewerComponent(embed.viewer, embed.file, embed.subpath),
	);
}

const reloadPDFViewerComponent = (
	...
	const oldEscapeHandler = viewer.scope.keys.find(
		(handler) => handler.modifiers === "" && handler.key === "Escape",
	);
...
const patchPDFViewerComponent = (plugin: PDFReader, pdfViewerComponent: PDFViewerComponent) => {
	plugin.register(
		around(pdfViewerComponent.constructor.prototype, {
			loadFile(old) {
				return async function (this: PDFViewerComponent, file: TFile, subpath?: string) {
					...
					this.then((child) => {
						if (!this.visualizer || this.visualizer.file !== file) {
							...
							this.visualizer = this.addChild(
								PDFViewerBacklinkVisualizer.create(plugin, file, child),
							);
						}
					});
...
const patchPDFViewerChild = (plugin: PDFReader, child: PDFViewerChild) => {
	const { app, lib } = plugin;

	plugin.register(
		around(child.constructor.prototype, {
			load(old) {
				return async function (this: PDFViewerChild, ...args: any[]) {
					...
					if (viewerContainerEl) {
						...
						const updateIsModEvent = (evt: MouseEvent) => {
							isModEvent ||=
								isModifierName(plugin.settings.showContextMenuOnMouseUpIf) &&
								Keymap.isModifier(evt, plugin.settings.showContextMenuOnMouseUpIf);
						};
						...
						const fixTextSelection = (evt: PointerEvent) => {
							...
							// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/476
							...
							if (
								...
								const startPageEl =
									plugin.lib.getPageElAssociatedWithNode(startContainer);
								const endPageEl =
									plugin.lib.getPageElAssociatedWithNode(endContainer);
								...
								const lastTextLayerNode = Array.from(
									startPageEl.querySelectorAll(".textLayerNode"),
								).at(-1);
								if (lastTextLayerNode) {
									...
									selection.setBaseAndExtent(
										...
										startOffset,
										lastTextLayerNode,
										lastTextLayerNode.childNodes.length,
									);
									return;
								}
							}
						...
						const onPointerUp = (evt: PointerEvent) => {
							...
							if (
								plugin.obsidianHasTextSelectionBug &&
								plugin.settings.fixObsidianTextSelectionBug
							) {
							...
							if (this.palette?.selectedColorName !== undefined) {
								if (evt.win.getSelection()?.toString()) {
									// If it's a double click, wait for selectDoubleClickedWord to finish expansion
									const delay = evt.detail > 1 ? 50 : 0;
									evt.win.setTimeout(() => {
										if (
											this.palette?.selectedColorName !== undefined &&
											evt.win.getSelection()?.toString()
										) {
											this.palette.applyColorToSelection(
												this.palette.selectedColorName,
											);
											this.palette.setActiveItem(undefined);
										}
									}, delay);
									return;
								}
							...
							if (plugin.settings.selectionToolbar) {
								if (plugin.settings.showSelectionToolbarOnMouseUp || isModEvent) {
									if (evt.win.getSelection()?.toString()) {
										if (this.component && !this.selectionToolbar) {
											this.selectionToolbar = this.component.addChild(
												new SelectionToolbar(plugin, this),
											);
										}
										// Use a slightly longer delay for double-clicks to catch the expansion
										const delay = evt.detail > 1 ? 120 : 80;
										evt.win.setTimeout(
											() => this.selectionToolbar?.show(evt),
											delay,
										);
										return; // Stop here, no context menu popup on selection
									} else {
							...
							// We no longer automatically show the context menu on selection by default
							// as per the user's request. Manual right-click still works.
					...
					const addColorPaletteToToolbar = () => {
						try {
							if (this.toolbar) {
								plugin.domManager.addChild(
									new PDFReaderToolbar(plugin, this.toolbar, this),
								);
							} else {
							...
							new Notice(
								`${plugin.manifest.name}: An error occurred while mounting the color palette to the toolbar.`,
							);
							console.error(e);
						}
					};
					...
					if (
						!requireApiVersion(fixedApiVersion) &&
						findNextIconEl &&
			...
			onResize(old) {
				return function (this: PDFViewerChild) {
					const pdfContainerEl =
						this.containerEl.querySelector<HTMLElement>(".pdf-container");
					if (pdfContainerEl) {
			...
			loadFile(old) {
				return async function (this: PDFViewerChild, file: TFile, subpath?: string) {
					...
					// - TypeError: Cannot read properties of null (reading 'eventBus')
					// (also reported in https://github.com/RyotaUshio/obsidian-pdf-reader/issues/315)
					...
					if (file.stat.size < 300) {
						...
						if (redirectTo) {
							const redirectFrom = app.vault
								.getResourcePath(file)
								.replace(/\?\d+$/, "");
							this.pdfViewer.pdfReaderRedirect = {
								from: redirectFrom,
								to: redirectTo,
							};
						...
						(data) => {
							...
							pageView.annotationLayer?.div
								...
								.forEach((el) => {
									...
									const annot =
										pageView.annotationLayer?.annotationLayer.getAnnotation(
											annotationId,
										);
									if (!annot) return;
									...
									if (
										annot.container.dataset
											.pdfReaderIsAnnotationPostProcessed === "true"
									)
										return;

									if (
										annot.data.subtype === "Link" &&
										typeof annot.container.dataset.internalLink === "string"
									) {
										PDFInternalLinkPostProcessor.registerEvents(
											plugin,
											this,
											annot,
										);
									} else if (annot.data.subtype === "Link" && annot.data.url) {
										PDFExternalLinkPostProcessor.registerEvents(
											plugin,
											this,
											annot,
										);
									...
									// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/68
									if (
										(plugin.settings.hideReplyAnnotation &&
											annot.data.inReplyTo &&
											annot.data.replyType === "R") ||
										(annot.data.subtype === "Stamp" &&
											plugin.settings.hideStampAnnotation)
									) {
										annot.container.hide();
									}

									if (
										!Platform.isPhone && // Without this, tapping on an annotation on mobile opens two annotation popups (which is a "Modal" instance with ""pdf-annotation-modal" class added to its containerEl)
										plugin.settings.showAnnotationPopupOnHover &&
										annot.data.contentsObj?.str
									) {
										showChildElOnParentElHover({
											...
											removeChildEl: () => {
												if (
													this.activeAnnotationPopupEl?.dataset
														.annotationId === annot.data.id
												) {
													this.destroyAnnotationPopup();
												}
											},
									...
									annot.container.dataset.pdfReaderIsAnnotationPostProcessed =
										"true";
								});
						},
					);
					...
					lib.registerPDFEvent(
						...
						async (data: {
							...
							outlineCount: number;
							currentOutlineItemPromise: Promise<void>;
						}) => {
							...
							pdfOutlineViewer.allItems.forEach((item) =>
								PDFOutlineItemPostProcessor.registerEvents(plugin, this, item),
							);

							if (plugin.settings.outlineContextMenu) {
								plugin.registerDomEvent(
									pdfOutlineViewer.childrenEl,
									"contextmenu",
									(evt) => {
					...
					// (A similar issue was reported before in https://github.com/RyotaUshio/obsidian-pdf-reader/issues/137)
					...
					lib.registerPDFEvent(
						...
						(data) => {
							...
							if (
								plugin.settings.noSidebarInEmbed &&
								!isNonEmbedLike(this.pdfViewer)
							) {
								pdfSidebar.close();
							}
							if (
								plugin.settings.defaultSidebarView === SidebarView.OUTLINE &&
								pdfSidebar.haveOutline
							) {
							...
							debounce(({ pageNumber }) => {
								if (plugin.settings.viewSyncFollowPageNumber) {
									...
									if (view && view.viewer.child === this) {
										const override = {
											state: { file: this.file!.path, page: pageNumber },
										};
										app.workspace.trigger(
											"view-sync:state-change",
											view,
											override,
										);
									}
					...
					if (this.pdfViewer.dom && this.component) {
						this.component.registerDomEvent(
							this.pdfViewer.dom.viewerEl,
							"dblclick",
							(evt) => {
								selectDoubleClickedWord(evt);
								// Force show selection toolbar after double click selection is ready
								evt.win.setTimeout(() => {
									this.selectionToolbar?.show(evt);
								}, 20);
							},
					...
					if (subpath) {
						...
						if (params.has("search") && this.findBar) {
							...
							if (plugin.settings.searchLinkHighlightAll !== "default") {
								settings.highlightAll =
									plugin.settings.searchLinkHighlightAll === "true";
							}
							if (plugin.settings.searchLinkCaseSensitive !== "default") {
								settings.caseSensitive =
									plugin.settings.searchLinkCaseSensitive === "true";
							}
							if (plugin.settings.searchLinkMatchDiacritics !== "default") {
								settings.matchDiacritics =
									plugin.settings.searchLinkMatchDiacritics === "true";
							}
							if (plugin.settings.searchLinkEntireWord !== "default") {
								settings.entireWord =
									plugin.settings.searchLinkEntireWord === "true";
							}
						...
						const { dest, highlight } = ((subpath) => {
							...
							let dest: [number, { name: string }, ...(number | null)[]] | null =
								null;
							...
							if (!dest) {
								if (params.has("offset")) {
									...
									!this.opts.isEmbed && // We need exclude embeds-likes (https://github.com/RyotaUshio/obsidian-pdf-reader/issues/137)
			...
			// See also https://github.com/RyotaUshio/obsidian-pdf-reader/issues/327
			...(apiVersion === "1.8.0"
				? {
						getTextSelectionRangeStr() {
							return function (this: PDFViewerChild, pageEl: HTMLElement) {
								...
								const textSelectionRange =
									selection &&
									lib.copyLink.getTextSelectionRange(pageEl, selection);
			...
			getPageLinkAlias(old) {
				return function (this: PDFViewerChild, page: number): string {
					if (this.file) {
						const alias = lib.copyLink.getDisplayText(
							this,
							undefined,
							this.file,
							page,
							lib.toSingleLine(activeWindow.getSelection()?.toString() ?? ""),
						);
			...
			highlightText(old) {
				return function (
					...
					if (textLayerInfo) {
						...
						if (
							plugin.settings.trimSelectionEmbed &&
							this.pdfViewer.isEmbed &&
							this.pdfViewer.dom &&
							!(
								plugin.settings.ignoreHeightParamInPopoverPreview &&
								this.pdfViewer.dom.containerEl.parentElement?.matches(
									".hover-popover",
								)
							)
						) {
							...
							if (textDivFirst && textDivLast) {
								setTimeout(() => {
									const containerRect =
										this.pdfViewer.dom!.viewerContainerEl.getBoundingClientRect();
									const firstRect = textDivFirst!.getBoundingClientRect();
									const lastRect = textDivLast.getBoundingClientRect();
									const height =
										lastRect.bottom -
										firstRect.top +
										2 * Math.abs(firstRect.top - containerRect.top);
					...
					if (
						!(
							plugin.settings.noTextHighlightsInEmbed &&
							this.pdfViewer.isEmbed &&
							!this.pdfViewer.dom?.containerEl.parentElement?.matches(
								".hover-popover",
							)
					...
					if (textDivFirst) {
						window.pdfjsViewer.scrollIntoView(
							textDivFirst,
							{
			...
			highlightAnnotation(old) {
				return function (this: PDFViewerChild, page: number, id: string) {
					const getAnnotationEl = () => {
						...
						const pageView = this.getPage(page);
						return pageView.annotationLayer?.div.querySelector<HTMLElement>(
						...
						setTimeout(() => {
							...
							if (el) {
								...
								const height =
									...
									annotationRect.top +
									2 * Math.abs(annotationRect.top - containerRect.top);
								this.pdfViewer.setHeight(height);
							}
						}, 100);
					}
					...
					if (
						!(
							plugin.settings.noAnnotationHighlightsInEmbed &&
							this.pdfViewer.isEmbed &&
							!this.pdfViewer.dom?.containerEl.parentElement?.matches(
								".hover-popover",
							)
			...
			clearTextHighlight(old) {
				return function (this: PDFViewerChild) {
					if (
						plugin.settings.persistentTextHighlightsInEmbed &&
						(this.pdfViewer?.isEmbed ?? this.opts.isEmbed)
					) {
						return;
					}
					old.call(this);
			...
			clearAnnotationHighlight(old) {
				return function (this: PDFViewerChild) {
					if (
						plugin.settings.persistentAnnotationHighlightsInEmbed &&
						this.pdfViewer.isEmbed
					) {
						return;
					}
			...
			renderAnnotationPopup(old) {
				return function (
					...
					const { page, id } =
						lib.getAnnotationInfoFromAnnotationElement(annotationElement);

					if (plugin.settings.renderMarkdownInStickyNote && this.file) {
						const contentEl =
							this.activeAnnotationPopupEl?.querySelector<HTMLElement>(
								".popupContent",
							);
						if (contentEl) {
						...
						popupMetaEl.createDiv(
							...
							(iconContainerEl) => {
									...
									iconContainerEl.createDiv(
										...
										(iconEl) => {
											...
											iconEl.addEventListener("click", async () => {
												const palette =
													lib.getColorPaletteAssociatedWithNode(
														popupMetaEl,
													);
												if (!palette) return;
												const template =
													plugin.settings.copyCommands[
														palette.actionIndex
													].template;

												lib.copyLink.copyLinkToAnnotation(
													this,
													false,
													{ copyFormat: template },
													page,
								...
								if (
									...
									PDFAnnotationEditModal.isSubtypeSupported(
										annotationElement.data.subtype,
									)
								) {
									...
									iconContainerEl.createDiv(
										...
										(editButtonEl) => {
											...
											editButtonEl.addEventListener("click", async () => {
												if (this.file) {
													PDFAnnotationEditModal.forSubtype(
														subtype,
														plugin,
														this.file,
														page,
					...
					if (Platform.isPhone) {
							...
							for (const mutation of mutations) {
								for (const node of mutation.addedNodes) {
									if (
										...
										node.matches("div.modal-container.pdf-annotation-modal")
									) {
										const popupMetaEl =
											node.querySelector<HTMLElement>(".popupMeta");
										if (popupMetaEl) {
						...
						const popupMetaEl =
							this.activeAnnotationPopupEl?.querySelector<HTMLElement>(".popupMeta");
						if (popupMetaEl) modifyAnnotationPopup(popupMetaEl);
					}

					if (
						plugin.settings.annotationPopupDrag &&
						this.activeAnnotationPopupEl &&
						this.file
					) {
						...
						for (let index = 0; index < items.length; index++) {
							...
							if (item.chars && item.chars.length) {
								...
								for (const {
									char,
									rect,
								} of getCharactersWithBoundingBoxesInPDFCoords(
									pageView,
									divs[index],
								)) {
...
const patchObsidianViewer = (plugin: PDFReader, pdfViewer: ObsidianViewer) => {
		...
		around(prototype, {
			open(old) {
				return async function (this: ObsidianViewer, args: any) {
					if (this.pdfReaderRedirect) {
						const { from, to } = this.pdfReaderRedirect;
						const url = args.url;
						if (
							typeof url === "string" &&
			...
			load(old) {
				return function (this: ObsidianViewer, doc: PDFDocumentProxy, ...args: any[]) {
					const callbacks = this.pdfReaderCallbacksOnDocumentLoaded;
					if (callbacks) {
						for (const callback of callbacks) {
							callback(doc);
						}
					}
					delete this.pdfReaderCallbacksOnDocumentLoaded;
...
const patchAppOptions = (plugin: PDFReader) => {
	plugin.register(
		around(window.pdfjsViewer.AppOptions, {
			get(old) {
				return function (...args: any[]) {
					const name = args[0];
					if (
						["defaultZoomValue", "scrollModeOnLoad", "spreadModeOnLoad"].includes(name)
					) {
						return plugin.settings[name as keyof PDFReaderSettings];
					}
