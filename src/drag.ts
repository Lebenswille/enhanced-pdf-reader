...
export const registerOutlineDrag = async (
	plugin: PDFReader,
	...
	for (const item of pdfOutlineViewer.allItems) {
		promises.push(
			(async () => {
				const textGenerator = await lib.copyLink.getTextToCopyForOutlineItemDynamic(
					child,
					file,
					item,
				);
							...
							if (!dragging) {
								(async () => {
									...
									if (!destItem || !itemToMove) {
										new Notice(
											`${plugin.manifest.name}: Failed to move the outline item.`,
										);
										return;
									}
									...
									await app.vault.modifyBinary(file, buffer as any);
								})();
							}
...
export const registerThumbnailDrag = (plugin: PDFReader, child: PDFViewerChild, file: TFile) => {
	const { app, lib } = plugin;
...
export const registerAnnotationPopupDrag = (
	plugin: PDFReader,
	popupEl: HTMLElement,
	child: PDFViewerChild,
	file: TFile,
	...
	child.getAnnotatedText(pageView, id).then((text): void => {
