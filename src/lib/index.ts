...
import { PDFReaderCommands } from "./commands";
import { PDFReaderSync } from "./sync";
...
type OptionalRenderParameters = Omit<
	RenderParameters,
	"canvas" | "canvasContext" | "viewport" | "transform"
> & {
...
export class PDFReaderLib {
	app: App;
	plugin: PDFReader;

	/** Sub-modules */
	commands: PDFReaderCommands;
	...
	constructor(plugin: PDFReader) {
		...
		this.commands = new PDFReaderCommands(plugin);
		...
		this.sync = new PDFReaderSync(plugin);
		this.dummyFileManager = new DummyFileManager(plugin);
		this.linkUpdater = new LinkUpdater(plugin);
	 ...
	 */
	registerPDFEvent<K extends keyof PDFJsEventMap>(
		name: K,
		eventBus: EventBus,
		component: Component | null,
		callback: (data: PDFJsEventMap[K]) => any,
	) {
	...
	onPageReady(
		...
		component: Component | null,
		cb: (pageNumber: number, pageView: PDFPageView, newlyRendered: boolean) => any,
	) {
		viewer.pdfViewer?._pages.forEach((pageView, pageIndex) => {
			cb(pageIndex + 1, pageView, false); // page number is 1-based
		});
		this.registerPDFEvent(
			"pagerendered",
			viewer.eventBus,
			component,
			(data: { source: PDFPageView; pageNumber: number }) => {
	...
	applyPDFViewStateToViewer(pdfViewer: PDFViewer, state: PDFViewExtraState) {
		...
		if (pdfViewer.pagesCount) {
			...
			// pages are not loaded yet (this is the case typically when opening a different file)
	...
	getColorPaletteOptions(): ColorPaletteState {
		...
		return {
			selectedColorName: [null, ...Object.keys(settings.colors)][
				settings.defaultColorPaletteItemIndex
			],
	...
	async ensureDestArray(dest: string | DestArray, doc: PDFDocumentProxy) {
		if (typeof dest === "string") {
			...
			dest = this.normalizePDFJsDestArray(
				destArray,
				(await doc.getPageIndex(destArray[0])) + 1,
			);
		}
	...
	registerGlobalDomEvent<K extends keyof DocumentEventMap>(
		component: Component,
		type: K,
	...
	generateMarkdownLink(file: TFile, sourcePath: string, subpath?: string, alias?: string) {
		...
		if (useMarkdownLinks) {
			nonEmbedLink = "["
				.concat(alias || file.basename, "](")
				.concat(encodeLinktext(linktext), ")");
		} else {
	...
	isBacklinked(
		...
		if (subpathParams) {
			...
			if (selection && (selection.length !== 4 || selection.some((pos) => isNaN(pos))))
				throw new Error("Invalid selection");
			if (selection && typeof annotation === "string")
				throw new Error("Selection and annotation cannot be used together");
		}
	...
	getPDFEmbedInMarkdownView(view: MarkdownView): PDFEmbed | null {
		return (
			this.getPDFEmbedsInComponent(view.currentMode as unknown as Component, true).first() ??
			null
		);
	}
	...
	getAllPDFEmbedsInCanvasView(view: CanvasView): PDFEmbed[] {
		return Array.from(view.canvas.nodes.values()).flatMap((node) => {
			if ("child" in node && node.child instanceof Component) {
	...
	getAllPDFEmbedsInExcalidrawView(view: ExcalidrawView): PDFEmbed[] {
		return Array.from(view.embeddableLeafRefs.values()).flatMap((ref) => {
			const node = ref.node;
			if (!node) return [];
			if ("child" in node && node.child instanceof Component) {
				return this.getPDFEmbedsInComponent(node.child, false);
	...
	search(
		findBar: PDFFindBar,
		query: string,
		settings?: Partial<PDFSearchSettings>,
	...
	updateSearchSettingsUI(findBar: PDFFindBar) {
		const toggleEls =
			findBar.settingsEl.querySelectorAll<HTMLElement>("div.checkbox-container");
		const highlightAllToggleEl = toggleEls[0];
		const matchDiacriticsToggleEl = toggleEls[1];
		const entireWordToggleEl = toggleEls[2];
		const caseSensitiveToggleIconEl = findBar.searchComponent.containerEl.querySelector(
			".input-right-decorator.clickable-icon",
		);

		if (highlightAllToggleEl)
			highlightAllToggleEl.toggleClass("is-enabled", findBar.searchSettings.highlightAll);
		if (matchDiacriticsToggleEl)
			matchDiacriticsToggleEl.toggleClass(
				"is-enabled",
				findBar.searchSettings.matchDiacritics,
			);
		if (entireWordToggleEl)
			entireWordToggleEl.toggleClass("is-enabled", findBar.searchSettings.entireWord);
		if (caseSensitiveToggleIconEl)
			caseSensitiveToggleIconEl.toggleClass(
				"is-active",
				findBar.searchSettings.caseSensitive,
			);
	}
		...
		if (content.startsWith("https://") || content.startsWith("http://")) {
			...
			if (res.status === 200) {
				const url = URL.createObjectURL(
					new Blob([res.arrayBuffer], { type: "application/pdf" }),
				);
				return url;
			}
	...
	async loadPDFDocumentFromArrayBufferOrUrl(
	...
	async loadPdfLibDocumentFromArrayBuffer(
		buffer: ArrayBuffer,
		...
		try {
			...
			if (e instanceof EncryptedPDFError) {
				new Notice(
					`${this.plugin.manifest.name}: The PDF file is encrypted. This plugin currently does not support editing encrypted PDF files.`,
					8,
				);
			}
			throw e;
	...
	async getPdfLibDocument(activeOnly: boolean = false) {
		...
		if (doc) {
			const u8 = await doc.getData();
			return await this.loadPdfLibDocumentFromArrayBuffer(u8.slice().buffer as ArrayBuffer);
		}
	}
	...
	async getPdfLibPage(activeOnly: boolean = false) {
		...
		const u8 = await pdfViewer.pdfDocument.getData();
		const doc = await this.loadPdfLibDocumentFromArrayBuffer(u8.slice().buffer as ArrayBuffer);
		if (doc) {
	 ...
	 */
	async pdfPageToImageDataUrl(
		page: PDFPageProxy,
		options?: PDFPageToImageOptions,
	): Promise<string> {
	 ...
	 */
	async pdfPageToImageArrayBuffer(
		page: PDFPageProxy,
		options?: PDFPageToImageOptions,
	): Promise<ArrayBuffer> {
	...
	getOptionalRenderParameters(): OptionalRenderParameters {
		return this.plugin.settings.rectFollowAdaptToTheme &&
			this.app.loadLocalStorage("pdfjs-is-themed")
			? {
	...
	getSelectedText(
		...
		if (beginIndex === endIndex) {
			return this.toSingleLine(
				textContentItems[beginIndex].str.slice(beginOffset, endOffset),
			);
		}
		const texts = [];
	...
	onDocumentReady(pdfViewer: ObsidianViewer, callback: (doc: PDFDocumentProxy) => any) {
		...
		// Callback functions in `pdfReaderCallbacksOnDocumentLoaded` are executed in `pdfViewer.load`.
		// See `patchObsidianViewer` in src/patchers/pdf-internals.ts.
		if (!pdfViewer.pdfReaderCallbacksOnDocumentLoaded) {
			pdfViewer.pdfReaderCallbacksOnDocumentLoaded = [];
		}
		pdfViewer.pdfReaderCallbacksOnDocumentLoaded.push(callback);
	}
	...
	async checkForUpdates(options?: {
		...
		const { result: response, error: requestError } = await tryCatchAsync(
			requestUrl({
				url: "https://api.github.com/repos/Lebenswille/enhanced-pdf-reader-for-obsidian/releases?per_page=10&page=1",
				headers: {
		...
		const { result: releases, error: jsonError } = tryCatchSync(
			() =>
		...
		const newerReleases = releases.filter((release) => {
			return (
				!release.prerelease &&
				!release.draft &&
				isVersionNewerThan(release.tag_name, currentVersion)
			);
		});
		...
		const hoursSinceNextRelease =
			(new Date().getTime() - new Date(nextRelease.published_at).getTime()) /
			(1000 * 60 * 60);
