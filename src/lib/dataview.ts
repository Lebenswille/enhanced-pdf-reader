...
const listItemContainsInlineFields = (item: ListItem, app: App, propertyName: string): boolean => {
	...
	values = values.filter(
		(value): value is Link =>
			value && value.hasOwnProperty("path") && value.hasOwnProperty("subpath"),
	);
	...
	const numFrontmatterLinks = frontmatterLinks
		? frontmatterLinks.filter(
				(link) => link.key === propertyName || link.key.startsWith(propertyName + "."),
			).length
		: 0;
...
export const withFilesWithInlineFields = (
	plugin: PDFReader,
	callback: (files: TFile[]) => void,
) => {
...
const getFilesWithInlineFields = async (plugin: PDFReader): Promise<TFile[]> => {
	const app = plugin.app;
...
export class DataviewInlineFieldsModal extends PDFReaderModal {
	filesWithInlineFields: TFile[];

	constructor(plugin: PDFReader, filesWithInlineFields: TFile[]) {
		super(plugin);
		this.filesWithInlineFields = filesWithInlineFields;
	}

	static async open(plugin: PDFReader, onlyIfNecessary = false) {
		withFilesWithInlineFields(plugin, (files) => {
			if (onlyIfNecessary && files.length === 0) {
				return;
			}

			new DataviewInlineFieldsModal(plugin, files).open();
		});
	}
	...
	onOpen() {
		...
		this.setTitle(
			`${this.plugin.manifest.name}: About the "${propertyName}" Dataview inline fields`,
		);
		this.renderContent();
		this.renderButtons();
	}
	...
	renderContent() {
		...
		this.contentEl.createEl("p", {
			text: createFragment((el) =>
				el.append(
					files.length >= 2
						? "The following files seem to have "
						: "The following file seems to have ",
					...
					createEl(
						...
						(a) => {
							setTooltip(a, "https://help.obsidian.md/properties", {
								placement: "top",
							});
						},
					),
		...
		this.contentEl.createDiv("modal-button-container", (el) => {
			if (files.length > 0) {
				new ButtonComponent(el)
					.setButtonText(
						files.length >= 2 ? "Copy links as markdown" : "Copy link as markdown",
					)
					.setCta()
					.onClick(async () => {
						const text =
			...
			new ButtonComponent(el).setButtonText("Close").onClick(() => {
				this.close();
			});
		});
	}
