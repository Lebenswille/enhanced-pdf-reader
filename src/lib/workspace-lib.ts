...
export type ExtendedPaneType =
	| Exclude<PaneType, "split">
	| "" // An empty string means the same as false (= current tab)
...
export function isExtendedPaneType(arg: string): arg is ExtendedPaneType {
	return (
		["", "tab", "window"].contains(arg) ||
		isFineGrainedSplitDirection(arg) ||
		isSidebarType(arg)
	);
}

export class WorkspaceLib extends PDFReaderLibSubmodule {
	...
	iterateBacklinkViews(cb: (view: BacklinkView) => any): void {
		this.app.workspace
			...
			.forEach((leaf) => cb(leaf.view as BacklinkView));
	}
	...
	iteratePDFViewerComponents(
		callback: (pdfViewerComponent: PDFViewerComponent, file: TFile | null) => any,
	): void {
		this.app.workspace.iterateAllLeaves((leaf) => {
			const view = leaf.view;
			...
			if (this.lib.isPDFView(view)) {
				...
				this.lib
					.getAllPDFEmbedsInMarkdownView(view)
					.forEach((embed) => callback(embed.viewer, embed.file));
			} else if (this.lib.isCanvasView(view)) {
				this.lib
					.getAllPDFEmbedsInCanvasView(view)
					.forEach((embed) => callback(embed.viewer, embed.file));
			} else if (this.lib.isExcalidrawView(view)) {
				this.lib
					.getAllPDFEmbedsInExcalidrawView(view)
					.forEach((embed) => callback(embed.viewer, embed.file));
			}
		});
	}
	...
	async openMarkdownLinkFromPDF(
		linktext: string,
		sourcePath: string,
		paneType: PaneType | boolean,
		position?: { pos: Pos } | { line: number },
	) {
		...
		if (paneType) {
			...
			if (
				isSidebarType(this.settings.paneTypeForFirstMDLeaf) &&
				this.settings.alwaysUseSidebar
			) {
		...
		if (
			!(
				markdownLeaf.parentSplit instanceof WorkspaceTabs &&
	...
	getMarkdownLeafInSidebar(sidebarType: SidebarType) {
		if (this.settings.singleMDLeafInSidebar) {
			return (
				this.lib.workspace.getExistingMarkdownLeafInSidebar(sidebarType) ??
	...
	getMarkdownLeafForLinkFromPDF(linktext: string, sourcePath: string): WorkspaceLeaf {
		...
		if (!markdownLeaf) {
			if (
				...
				markdownLeaf =
					this.getExistingMarkdownLeafInSidebar(this.settings.paneTypeForFirstMDLeaf) ??
	 ...
	 */
	openPDFLinkTextInExistingLeafForTargetPDF(
		linktext: string,
		sourcePath: string,
		openViewState?: OpenViewState,
		targetFile?: TFile,
	): { exists: boolean; promise: Promise<void> } {
		if (!targetFile) {
		...
		if (
			!(
				sameFileLeaf.parentSplit instanceof WorkspaceTabs &&
				sameFileLeaf.parentSplit === this.app.workspace.activeLeaf?.parentSplit
			)
		) {
			openViewState = openViewState ?? {};
		...
		const promise = this.openPDFLinkTextInLeaf(
			sameFileLeaf,
			linktext,
			sourcePath,
			openViewState,
	...
	getExistingLeafForFile(file: TFile): WorkspaceLeaf | null {
		...
		this.app.workspace.iterateAllLeaves((l) => {
			...
			if (
				l.view.getViewType() === viewType &&
				this.getFilePathFromView(l.view) === file.path
			) {
				leaf = l;
			}
		});
	...
	registerHideSidebar(leaf: WorkspaceLeaf) {
		...
		if (root === this.app.workspace.leftSplit || root === this.app.workspace.rightSplit) {
			const sidebar = root as
				| typeof this.app.workspace.leftSplit
				| typeof this.app.workspace.rightSplit;
 ...
 */
class HoverEditorLib extends PDFReaderLibSubmodule {
	...
	async createNewHoverEditorLeaf(
	...
	getHoverEditorForLeaf(leaf: WorkspaceLeaf) {
		return (
			this.hoverEditorPlugin?.activePopovers.find((popover) =>
