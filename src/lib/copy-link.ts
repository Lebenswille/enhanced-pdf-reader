...
import { PDFReaderTemplateProcessor } from "template";
import {
	encodeLinktext,
...
export class copyLinkLib extends PDFReaderLibSubmodule {
	statusDurationMs = 2000;

	getPageAndTextRangeFromSelection(
		...
		if (selection) {
			const selectionRange = this.getTextSelectionRange(pageEl, selection);
	...
	getTextSelectionRange(pageEl: HTMLElement, selection: Selection) {
		if (!selection || selection.rangeCount === 0 || selection.isCollapsed) return null;

		let totalBeginIndex: number | null = null;
		let totalBeginOffset: number | null = null;
		let totalEndIndex: number | null = null;
		let totalEndOffset: number | null = null;
		...
		for (let i = 0; i < selection.rangeCount; i++) {
			...
			if (startNode && endNode) {
				const bIdx = +startNode.dataset.idx! - this.plugin.textDivFirstIdx;
				const eIdx = +endNode.dataset.idx! - this.plugin.textDivFirstIdx;
				const bOff = getOffsetInTextLayerNode(
					startNode,
					range.startContainer,
					range.startOffset,
				);
				const eOff = getOffsetInTextLayerNode(endNode, range.endContainer, range.endOffset);

				if (bOff !== null && eOff !== null) {
					if (
						totalBeginIndex === null ||
						bIdx < totalBeginIndex ||
						(bIdx === totalBeginIndex && bOff < totalBeginOffset!)
					) {
						totalBeginIndex = bIdx;
						totalBeginOffset = bOff;
					}
					if (
						totalEndIndex === null ||
						eIdx > totalEndIndex ||
						(eIdx === totalEndIndex && eOff > totalEndOffset!)
					) {
						totalEndIndex = eIdx;
						totalEndOffset = eOff;
					}
		...
		if (
			totalBeginIndex !== null &&
			totalEndIndex !== null &&
			totalBeginOffset !== null &&
			totalEndOffset !== null
		) {
			return {
				beginIndex: totalBeginIndex,
				beginOffset: totalBeginOffset,
	...
	lastSelectionVariables: any = null;

	getTemplateVariables(subpathParams: Record<string, any>) {
		let selection = activeWindow.getSelection();

		// If selection is lost (likely due to input focus), fallback to our cache
		if ((!selection || !selection.toString()) && this.lastSelectionVariables) {
			const existingParams = Object.fromEntries(
				subpathToParams(this.lastSelectionVariables.subpath).entries(),
			);
			return {
				...this.lastSelectionVariables,
				subpath: paramsToSubpath({
					...existingParams,
					...subpathParams,
				}),
			};
		...
		const { beginIndex, beginOffset, endIndex, endOffset } = selectionRange;
		const selectionStr = `${beginIndex},${beginOffset},${endIndex},${endOffset}`;
		...
		return this.lastSelectionVariables;
	}
	...
	getLinkTemplateVariables(
		...
		const linkWithDisplay = this.lib
			.generateMarkdownLink(file, sourcePath, subpath, display || undefined)
			.slice(1);

		const linkToPage = this.app.fileManager
			.generateMarkdownLink(file, sourcePath, `#page=${page}`)
			.slice(1);
		// https://github.com/obsidianmd/obsidian-api/issues/154
		// const linkToPageWithDisplay = app.fileManager.generateMarkdownLink(file, sourcePath, `#page=${page}`, display).slice(1);
		const linkToPageWithDisplay = this.lib
			.generateMarkdownLink(file, sourcePath, `#page=${page}`, display || undefined)
			.slice(1);
	...
	getDisplayText(
		...
		if (!displayTextFormat) {
			...
			if (palette) {
				displayTextFormat =
					this.settings.displayTextFormats[palette.displayTextFormatIndex].template;
			} else {
				displayTextFormat =
					this.settings.displayTextFormats[this.settings.defaultDisplayTextFormatIndex]
						.template;
			}
		}

		try {
			return new PDFReaderTemplateProcessor(this.plugin, {
				file,
				page,
				pageCount: child.pdfViewer.pagesCount,
				pageLabel: child.getPage(page).pageLabel ?? "" + page,
			...
			console.error(err);
			new Notice(
				`${this.plugin.manifest.name}: Display text format is invalid. Error: ${err.message}`,
				3000,
			);
		}
	}
	 ...
	 * (See also: https://github.com/RyotaUshio/obsidian-pdf-reader/issues/260)
	 * @returns
	 */
	getTextToCopy(
		child: PDFViewerChild,
		...
		if (typeof comment !== "string") {
			...
			comment =
				...
				pageView?.annotationLayer?.annotationLayer?.getAnnotation(annotationId)?.data
					?.contentsObj?.str;
			comment = this.lib.toSingleLine(comment || "");
		}

		const processor = new PDFReaderTemplateProcessor(this.plugin, {
			file,
			page,
		...
		// If the comment was manually provided but not included in the template, append it at the end
		if (typeof comment === "string" && comment.trim() && !template.includes("{{comment}}")) {
			return evaluated.trimEnd() + "\n> " + comment.trim() + "\n";
		}

		return evaluated;
	}

	async getTextToCopyForOutlineItem(
		child: PDFViewerChild,
		file: TFile,
	...
	async getTextToCopyForOutlineItemDynamic(
		child: PDFViewerChild,
		file: TFile,
		item: PDFOutlineTreeNode,
	) {
	...
	getAnnotationLinkInfo() {
		...
		const copyButtonEl = popupEl.querySelector<HTMLElement>(
			".popupMeta div.clickable-icon:has(svg.lucide-copy)",
		);
	...
	copyLinkToSelection(
		checking: boolean,
		templates: { copyFormat: string; displayTextFormat?: string },
		colorName?: string,
		autoPaste?: boolean,
		comment?: string,
			...
			if (!checking) {
				(async () => {
					const evaluated = this.getTextToCopy(
						child,
						templates.copyFormat,
						templates.displayTextFormat,
						file,
						page,
						subpath,
						text,
					...
					const shouldAutoPaste = autoPaste ?? this.settings.autoPaste;
					if (!shouldAutoPaste) {
						...
						// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/93
							...
							if (textLayerInfo) {
								...
								if (item) {
									...
									if (typeof left === "number" && typeof top === "number") {
										this.plugin.lastCopiedDestInfo = {
											file,
											destArray: [page - 1, "XYZ", left, top, null],
										};
									}
								}
							}
	...
	copyLinkToAnnotation(
		...
		if (!checking) {
			const pageView = child.getPage(page);
			child.getAnnotatedText(pageView, id).then(async (text) => {
				const annotData =
					pageView.annotationLayer?.annotationLayer?.getAnnotation(id)?.data ??
					(await pageView.pdfPage.getAnnotations()).find((annot) => annot.id === id);
				const color = annotData?.color
					? `${annotData.color[0]}, ${annotData.color[1]}, ${annotData.color[2]}`
					: "";
	...
	copyLinkToAnnotationWithGivenTextAndFile(
		text: string,
		file: TFile,
		child: PDFViewerChild,
		checking: boolean,
	...
	writeHighlightAnnotationToSelectionIntoFileAndCopyLink(
		checking: boolean,
		templates: { copyFormat: string; displayTextFormat?: string },
		colorName?: string,
		autoPaste?: boolean,
		...
		if (!checking) {
			...
			this.lib.highlight.writeFile
				.addTextMarkupAnnotationToSelection(
				...
				.then((result) => {
					if (!result) {
						palette?.setStatus("Failed to write annotation", 3000);
						return;
					}
					...
					setTimeout(() => {
						...
						const rgbStr = rgb ? `${rgb.r}, ${rgb.g}, ${rgb.b}` : "255, 255, 0";

						this.copyLinkToAnnotationWithGivenTextAndFile(
							text,
							file,
							child,
							false,
				...
				.catch((err) => {
					console.error(err);
					palette?.setStatus("Error writing annotation", 3000);
				});
		}
		return true;
	}
	...
	copyEmbedLinkToRect(
		...
		if (!checking) {
			...
			const embedLink = this.lib.generateMarkdownLink(
				file,
				sourcePath ?? "",
				subpath,
				display,
			...
			(async () => {
				...
				if (!this.settings.rectEmbedStaticImage) {
					...
					const imageEmbedLink = useWikilinks
						? `![[${imagePath}]]`
						: `![](${encodeLinktext(imagePath)})`;
					text = imageEmbedLink + "\n\n" + embedLink.slice(1);
					...
					const createImageFile = async () => {
						...
						// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/323
						//
						...
						// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/326
				...
				this.plugin.lastCopiedDestInfo = {
					file,
					destArray: [pageNumber - 1, "FitR", ...rect],
				};
	...
	copyLinkToSearch(
		...
		if (!checking) {
			const display = this.lib.copyLink.getDisplayText(
				child,
				undefined,
				file,
				pageNumber,
			...
			const link = this.lib
				.generateMarkdownLink(file, "", `#search=${query}`, display)
				.slice(1);
	...
	makeCanvasTextNodeFromSelection(
		checking: boolean,
		canvas: Canvas,
		template: string,
		colorName?: string,
	...
	makeCanvasTextNodeFromAnnotation(
		checking: boolean,
		canvas: Canvas,
		child: PDFViewerChild,
		template: string,
		...
		if (!checking) {
			const pageView = child.getPage(page);
			child.getAnnotatedText(pageView, id).then((text) => {
				const evaluated = this.getTextToCopy(
					child,
					template,
					undefined,
	...
	async autoPaste(
		...
		if (file) {
			// auto-paste target found
			// Check for duplicates if it's an associated note sync
		...
		const command = this.app.commands.findCommand(
			this.settings.commandToExecuteWhenTargetNotIdentified,
		);
		if (!command) {
			new Notice(
				`${this.plugin.manifest.name}: Command "${this.settings.commandToExecuteWhenTargetNotIdentified}" was not found. Please update the "Command to execute when pasting a link for the first time with auto-focus or auto-paste" setting.`,
			);
			return false;
		}
		...
		return new Promise<boolean>((resolve) => {
			...
			activeWindow.setTimeout(() => {
				if (!isResolved) {
					const { noticeEl } = new Notice(
						`${this.plugin.manifest.name}: Could not find the auto-paste target markdown file within ${this.settings.autoPasteTargetDialogTimeoutSec} seconds.`,
					);
	...
	async getAutoFocusOrAutoPasteTarget(
		...
		} else if (target === "last-paste-then-last-active")
			targetFile = lastPasteFile ?? lastActiveFile;
	...
	async prepareMarkdownLeafForPaste(file: TFile) {
		...
		const shouldOpen =
			!leaf && this.settings.openAutoFocusTargetIfNotOpened && !this.settings.autoSync;

		if (!leaf && shouldOpen) {
			const paneType = this.settings.howToOpenAutoFocusTargetIfNotOpened;

			if (paneType === "hover-editor") {
				const hoverLeaf = await this.lib.workspace.hoverEditor.createNewHoverEditorLeaf(
					{ hoverPopover: null },
					null,
					file.path,
					"",
	...
	async pasteTextToFile(text: string, file: TFile, forceUseVault = false) {
		// Ensure the cursor lands entirely outside the callout by appending a blank line
		...
		if (
			...
			// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/71
			const view = leaf.view;
	...
	async updateAndRevealCursorInEditor(
		view: MarkdownView,
		options: { focus: boolean; goEnd: boolean },
	) {
		...
		if (coords) {
			...
			if (
				coords.top < scrollInfo.top ||
				coords.top > scrollInfo.top + scrollInfo.clientHeight
			) {
				...
				// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/142
	...
	watchPaste(text: string, onPaste?: () => any) {
		...
		this.plugin.registerOneTimeEvent(
			this.app.workspace,
			"editor-paste",
			(evt: ClipboardEvent, editor: Editor, info: MarkdownView | MarkdownFileInfo) => {
				...
				// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/54
	...
	async autoFocusOrAutoPaste(
		...
		if (autoPaste || this.settings.autoPaste) {
			...
			if (success) {
				...
				if (
					!this.settings.focusEditorAfterAutoPaste &&
					this.settings.clearSelectionAfterAutoPaste
				) {
					const selection = activeWindow.getSelection();
					if (
						selection &&
						this.lib.copyLink.getPageAndTextRangeFromSelection(selection)
					) {
						selection.empty();
					}
				}
