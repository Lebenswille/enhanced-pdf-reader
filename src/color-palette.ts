...
import {
	...
	getEventCoords,
	isHexString,
	showMenuUnderParentEl,
	isTargetHTMLElement,
} from "utils";
import { PDFViewerChild, Rect } from "typings";
import { PDFReaderComponent } from "lib/component";

export type ColorPaletteState = Pick<
	ColorPalette,
	"selectedColorName" | "actionIndex" | "displayTextFormatIndex" | "writeFile"
>;
...
export class ColorPalette extends PDFReaderComponent {
	...
	constructor(plugin: PDFReader, child: PDFViewerChild, toolbarLeftEl: HTMLElement) {
		super(plugin);
		this.child = child;
	...
	onload() {
		...
		if (
			!this.plugin.settings.colorPaletteInEmbedToolbar &&
			this.toolbarLeftEl.closest(".pdf-embed")
		)
			return;
		...
		if (this.plugin.settings.colorPaletteInToolbar) {
			...
			if (this.plugin.settings.defaultColorPaletteItemIndex > 0) {
				this.setActiveItem(
					[null, ...Object.keys(this.plugin.settings.colors)][
						this.plugin.settings.defaultColorPaletteItemIndex
					],
				);
			} else {
		...
		itemEl.addEventListener("contextmenu", () => {
			...
			const menu = new Menu().addItem((item) => {
				item.setIcon("lucide-settings")
					.setTitle("Customize...")
					.onClick(() => {
						this.plugin.openSettingTab().scrollTo("colors");
					});
			});
			menu.onHide(() => {
				shown = false;
	...
	onItemPointerUp(itemEl: HTMLElement, name: string | null, evt: MouseEvent) {
		const colorChangedFromInactiveToActive = !itemEl.hasClass("is-active");
		if (colorChangedFromInactiveToActive) {
		...
		if (
			this.plugin.settings.syncColorPaletteItem &&
			this.plugin.settings.syncDefaultColorPaletteItem
		) {
			this.plugin.settings.defaultColorPaletteItemIndex =
				this.selectedColorName !== undefined
					? name
						? Object.keys(this.plugin.settings.colors).indexOf(name) + 1
						: 0
					: -1;
		}
		...
		if (colorChangedFromInactiveToActive) {
			...
			this.applyColorToSelection(name);

			// Only deselect if there was an active selection when clicking the button
			if (this.child.containerEl.win.getSelection()?.toString()) {
				this.setActiveItem(undefined);
			}
		}
	...
	setActiveItem(name: string | null | undefined) {
		this.selectedColorName = name === null || name === undefined ? name : name.toLowerCase();
		this.itemEls.forEach((el) => {
			el.toggleClass(
				"is-active",
				this.selectedColorName === (el.dataset.highlightColor ?? null),
			);
		});
	}

	applyColorToSelection(name: string | null) {
		const template = this.plugin.settings.copyCommands[this.actionIndex].template;
		const comment = this.child.selectionToolbar?.comment;
		...
		this.child.selectionToolbar?.hide();
	}

	addDropdown(
		paletteEl: HTMLElement,
		itemNames: string[],
		checkedIndexKey: KeysOfType<ColorPalette, number>,
		tooltip: string,
		onItemClick?: () => void,
		beforeShowMenu?: (menu: Menu) => void,
	) {
						...
						item.setTitle(name)
							...
							.onClick(() => {
								...
								if (checkedIndexChanged) {
									this.plugin.trigger("color-palette-state-change", {
										source: this,
									});
								}
							});
	...
	addCopyActionDropdown(paletteEl: HTMLElement) {
		...
		const buttonEl = this.addDropdown(
			...
			() => {
				this.updateTooltips();
				if (
					this.plugin.settings.syncColorPaletteAction &&
					this.plugin.settings.syncDefaultColorPaletteAction
				) {
			...
			(menu) => {
				menu.addItem((item) => {
					item.setTitle("Customize...")
						...
						.onClick(() => {
							this.plugin.openSettingTab().scrollTo("copyCommands");
						});
				});
			},
		);
		...
		const buttonEl = this.addDropdown(
			...
			() => {
				if (
					...
					this.plugin.settings.defaultDisplayTextFormatIndex =
						this.displayTextFormatIndex;
				}
			},
			(menu) => {
				menu.addItem((item) => {
					item.setTitle("Customize...")
						.setIcon("lucide-settings")
						.onClick(() => {
							this.plugin.openSettingTab().scrollTo("displayTextFormats");
						});
				});
			},
		);
	...
	addWriteFileToggle(paletteEl: HTMLElement) {
		...
		this.writeFileButtonEl = paletteEl.createDiv("clickable-icon", (el) => {
			...
			setTooltip(
				el,
				`Add ${this.plugin.settings.selectionBacklinkVisualizeStyle}s to file directly`,
			);
			...
			el.addEventListener("click", () => {
				if (!this.lib.isEditable(this.child)) {
					...
					const menu = new Menu().addItem((item) => {
						item.setIcon("lucide-settings")
							...
							.onClick(() => {
								this.plugin.openSettingTab().scrollToHeading("edit");
							});
					});
					menu.onHide(() => {
						shown = false;
				...
				if (
					this.plugin.settings.syncWriteFileToggle &&
					this.plugin.settings.syncDefaultWriteFileToggle
				) {
				...
				if (this.lib.isEditable(this.child)) {
					menu.addItem((item) => {
						item.setIcon("lucide-settings")
							.setTitle("Customize...")
							.onClick(() => {
								this.plugin.openSettingTab().scrollToHeading("annot");
							});
					});
				}
				menu.addItem((item) => {
					item.setIcon("lucide-settings")
						.setTitle(
							this.lib.isEditable(this.child)
								? "Disable PDF editing..."
								: "Enable PDF editing...",
						)
						.onClick(() => {
	...
	async importFile() {
		...
		if (!Platform.isDesktopApp && url.startsWith(Platform.resourcePathPrefix)) {
			new Notice(
				`${this.plugin.manifest.name}: Importing local PDFs outside the vault is supported only on the desktop app.`,
			);
			return;
		}
		...
		if (res.ok) {
			...
			new Notice(
				`${this.plugin.manifest.name}: Successfully imported the PDF file into the vault.`,
			);
			return;
		}
			...
			el.addEventListener("contextmenu", () => {
				...
				const menu = new Menu().addItem((item) => {
					item.setIcon("lucide-settings")
						.setTitle("Customize...")
						.onClick(() => {
							this.plugin.openSettingTab().scrollToHeading("rect");
						});
				});
				menu.onHide(() => {
					shown = false;
	...
	setTooltipToActionItem(itemEl: HTMLElement, name: string | null) {
		const pickerEl = itemEl.querySelector<HTMLInputElement>(
			":scope > ." + ColorPalette.CLS + "-item-inner",
		)!;
		...
		const tooltip =
			name !== null
				...
				: quiet
					? "No color specified"
					: `Copy link with "${commandName}" format without specifying color`;
	...
	setState(state: Partial<ColorPaletteState>) {
		if (typeof state.selectedColorName === "string")
			this.setActiveItem(state.selectedColorName);
		if (typeof state.actionIndex === "number") this.setActionIndex(state.actionIndex);
		if (typeof state.displayTextFormatIndex === "number")
			this.setDisplayTextFormatIndex(state.displayTextFormatIndex);
		if (typeof state.writeFile === "boolean") this.setWriteFile(state.writeFile);
	}

	getColorName(): string | null {
		const state = this.getState();
		return state.selectedColorName ?? null;
	}
