...
interface ObsidianViewer {
	...
	pdfReaderRedirect?: { from: string; to: string };
	pdfReaderCallbacksOnDocumentLoaded?: ((doc: PDFDocumentProxy) => any)[];
}
...
interface PDFViewer {
	...
	scrollPageIntoView(params: {
		pageNumber: number;
		destArray?: [number, { name: string }, ...number[]] | null;
...
type PDFJsDestArray = [
	pageRef: { num: number; gen: number },
	destType: { name: string },
	...params: (number | null)[],
];
type PdfLibDestArray = [
	pageRef: PDFRef,
	destType: PDFName,
	...params: (PDFNumber | typeof PDFNull)[],
];
...
interface DragManager {
	...
	updateSource(
		...args:
			| [sourceEls: HTMLElement[], sourceClass: "is-being-dragged"]
			| [sourceEls: null, sourceClass: ""]
	): void;
