...
export class PDFBacklinkVisualizer extends PDFReaderComponent {
	file: TFile;
	_index?: PDFBacklinkIndex;

	constructor(plugin: PDFReader, file: TFile) {
		super(plugin);
		this.file = file;
	}
...
export class BacklinkDomManager extends PDFReaderComponent {
	visualizer: PDFViewerBacklinkVisualizer;

	private pagewiseCacheToDomsMap = new Map<
		number,
		BidirectionalMultiValuedMap<PDFBacklinkCache, HTMLElement>
	>();
	private pagewiseStatus = new Map<
		number,
		{ onPageReady: boolean; onTextLayerReady: boolean; onAnnotationLayerReady: boolean }
	>();
	...
	updateStatus(
		...
		update: {
			onPageReady?: boolean;
			onTextLayerReady?: boolean;
			onAnnotationLayerReady?: boolean;
		},
	) {
		const status = this.getStatus(pageNumber);
	...
	hookBacklinkOpeners(el: HTMLElement, cache: PDFBacklinkCache) {
		...
		this.registerDomEventForCache(cache, el, "dblclick", (event) => {
			if (this.plugin.settings.doubleClickHighlightToOpenBacklink) {
				...
				this.lib.workspace.openMarkdownLinkFromPDF(
					cache.sourcePath,
					this.file.path,
					paneType,
					pos ? { pos } : undefined,
	...
	registerDomEventForCache<K extends keyof HTMLElementEventMap>(
 ...
 *
 * See also: https://github.com/RyotaUshio/obsidian-pdf-reader/issues/148
 */
export class RectangleCache extends PDFReaderComponent {
	visualizer: PDFViewerBacklinkVisualizer;
	...
	computeRectsForSelection(pageNumber: number, id: string) {
		...
		const { beginIndex, beginOffset, endIndex, endOffset } =
			PDFPageBacklinkIndex.selectionIdToParams(id);
...
export class PDFViewerBacklinkVisualizer extends PDFBacklinkVisualizer implements HoverParent {
	...
	constructor(plugin: PDFReader, file: TFile, child: PDFViewerChild) {
		super(plugin, file);
		this.child = child;
	}

	static create(plugin: PDFReader, file: TFile, child: PDFViewerChild) {
	...
	shouldVisualizeBacklinks(): boolean {
		...
		return (
			this.settings.highlightBacklinks &&
	...
	processFitR(pageNumber: number, id: string, caches: Set<PDFBacklinkCache>) {
		...
		const rectEl = this.lib.highlight.viewer.placeRectInPage(
	...
	showIcon(x: number, y: number, pageView: PDFPageView, side: "left" | "right" = "right") {
		const iconSize =
			(Math.min(
				(pageView.viewport.rawDims as any).pageWidth,
				(pageView.viewport.rawDims as any).pageWidth,
			) *
				this.settings.backlinkIconSize) /
			2000;
		const rectRight: Rect = [x, y - iconSize, x + iconSize, y];
		const rectLeft: Rect = [x - iconSize, y - iconSize, x, y];
		const rect =
			side === "left" && rectLeft[0] >= ((pageView.viewport.rawDims as any).pageX ?? 0)
				? rectLeft
				: rectRight;
