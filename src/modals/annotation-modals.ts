...
import { PDFReaderModal } from "modals";

class PDFAnnotationModal extends PDFReaderModal {
	file: TFile;
	page: number;
	id: string;

	constructor(plugin: PDFReader, file: TFile, page: number, id: string) {
		super(plugin);
		this.file = file;
		this.page = page;
		this.id = id;
...
export class PDFAnnotationEditModal extends PDFAnnotationModal {
	static readonly supportedSubtypes = [
		...
		"Underline",
		"Squiggly",
		"StrikeOut", // text markup annotations
		"Link",
	] as const;
	...
	static isSubtypeSupported(
		subtype: string,
	): subtype is (typeof PDFAnnotationEditModal.supportedSubtypes)[number] {
		return (PDFAnnotationEditModal.supportedSubtypes as unknown as string[]).includes(subtype);
	}

	static forSubtype(
		subtype: (typeof PDFAnnotationEditModal.supportedSubtypes)[number],
	...
	static forTextMarkupAnnotation(
		...args: ConstructorParameters<typeof PDFAnnotationModal>
	): PDFAnnotationEditModal {
	...
	static forLinkAnnotation(
		...args: ConstructorParameters<typeof PDFAnnotationModal>
	): PDFAnnotationEditModal {
		return new PDFAnnotationEditModal({ color: true, borderWidth: true }, ...args);
	}
	...
	async readOldValues() {
		const pdflibAPI = this.lib.highlight.writeFile;
		...
		for (const key of this.supportedKeys) {
			switch (key) {
				case "color":
					this.oldValues.color = pdflibAPI.getColorFromAnnotation(annot) ?? undefined;
					break;
				case "opacity":
					this.oldValues.opacity = pdflibAPI.getOpacityFromAnnotation(annot) ?? undefined;
					break;
				case "borderWidth":
					this.oldValues.borderWidth =
						pdflibAPI.getBorderWidthFromAnnotation(annot) ?? undefined;
					break;
				case "author":
					this.oldValues.author = pdflibAPI.getAuthorFromAnnotation(annot) ?? undefined;
					break;
				case "contents":
					this.oldValues.contents =
						pdflibAPI.getContentsFromAnnotation(annot) ?? undefined;
					break;
			}
		}
	}
	...
	async writeNewValues() {
		...
		for (const key of this.supportedKeys) {
			switch (key) {
				case "color":
					if (
						this.newValues.color &&
						JSON.stringify(this.newValues.color) !==
							JSON.stringify(this.oldValues.color)
					) {
				...
				case "opacity":
					if (
						typeof this.newValues.opacity === "number" &&
						this.newValues.opacity !== this.oldValues.opacity
					) {
						writers.push((annot: PDFDict) => {
				...
				case "borderWidth":
					if (
						typeof this.newValues.borderWidth === "number" &&
						this.newValues.borderWidth !== this.oldValues.borderWidth
					) {
						writers.push((annot: PDFDict) => {
							pdflibAPI.setBorderWidthToAnnotation(
								annot,
								this.newValues.borderWidth!,
							);
						});
					}
					break;
				...
				case "contents":
					if (
						typeof this.newValues.contents === "string" &&
						this.newValues.contents !== this.oldValues.contents
					) {
						writers.push((annot: PDFDict) => {
		...
		if (writers.length) {
			await pdflibAPI.processAnnotation(
				this.file,
				this.page,
				this.id,
				async (annot: PDFDict) => {
	...
	addColorSetting() {
		if (this.oldValues.color || this.allowNoValue.color) {
			...
			new Setting(this.contentEl)
				.setName("Color")
				.setDesc(
					"You can choose a color from the color picker or select one from your custom named colors.",
				)
				...
				.addDropdown((_dropdown) => {
					...
					dropdown
						.addOptions(
							Object.fromEntries(
								Object.entries(this.plugin.settings.colors).map(([name, value]) => [
									value,
									name,
								]),
							),
	...
	addOpacitySetting() {
		if (this.oldValues.opacity || this.allowNoValue.opacity) {
			new Setting(this.contentEl).setName("Opacity").addSlider((slider) => {
				slider
					.setLimits(0, 1, 0.01)
	...
	addBorderWidthSetting() {
		if (this.oldValues.borderWidth || this.allowNoValue.borderWidth) {
			new Setting(this.contentEl).setName("Draw border").addToggle((toggle) => {
				toggle.setValue(!!(this.oldValues.borderWidth ?? 1)).onChange((value) => {
	...
	addAuthorSetting() {
		if (this.oldValues.author || this.allowNoValue.author) {
			new Setting(this.contentEl).setName("Annotation author").addText((text) => {
				text.setValue(this.oldValues.author ?? "Author").onChange((value) => {
	...
	initContentsSetting() {
		...
		if (this.plugin.settings.renderMarkdownInStickyNote) {
			const hotkeys =
				this.app.hotkeyManager.getHotkeys("markdown:toggle-preview") ??
	...
	addContentsSetting() {
		if (this.oldValues.contents || this.allowNoValue.contents) {
			new Setting(this.contentEl)
				.setName("Comment")
				.then((setting) => {
					this.previewEl = setting.controlEl.createDiv(
						"preview-container markdown-rendered",
					);
					if (this.plugin.settings.renderMarkdownInStickyNote) {
						setting.setDesc(
							`Press ${this.app.hotkeyManager.printHotkeyForCommand("markdown:toggle-preview")} to toggle preview.`,
						);
					} else {
						setting.setDesc(
							createFragment((el) => {
								const anchorEl = createEl(
									...
									(el) => {
										el.onclick = (evt) => {
											...
											tab.scrollTo("renderMarkdownInStickyNote", {
												behavior: "smooth",
											});
										};
									},
				...
				.addTextArea((textarea) => {
					...
					textarea.setValue(this.oldValues.contents ?? "").onChange((value) => {
...
export class PDFAnnotationDeleteModal extends PDFAnnotationModal {
	onOpen() {
		...
		if (!this.plugin.settings.warnEveryAnnotationDelete) {
			this.contentEl.createEl("p", {
				cls: "mod-warning",
				text: "There are one or more links pointing to this annotation.",
			});
		}
	...
	shouldOpen() {
		return (
			this.plugin.settings.warnEveryAnnotationDelete ||
