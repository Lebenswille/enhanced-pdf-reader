...
export class DummyFileModal extends PDFReaderModal {
	...
	constructor(...args: ConstructorParameters<typeof PDFReaderModal>) {
		super(...args);
	...
	onOpen() {
		...
		this.folderPath = this.lib.dummyFileManager.getFolderPathForDummyFiles(
			this.app.workspace.getActiveFile(),
		);

		this.titleEl.setText(`${this.plugin.manifest.name}: Create dummy file for external PDF`);
		this.modalEl.createDiv("", (div) => {
			new Setting(div).setDesc(
				createFragment((el) => {
					const keys = this.plugin.settings.modifierToDropExternalPDFToCreateDummy;
					el.appendText(
						`You can also use ${keys.length ? keys.map(getModifierNameInPlatform).join("+") + " +" : ""} drag & drop to create dummy files. `,
					);
					el.createEl("a", {
		...
		for (let i = 0; i < this.uris.length; i++) {
			this.addSetting()
				...
				.addText((text) => {
					...
					if (this.source === "file") {
						text.setValue(
							this.uris[i] ? this.uris[i].replace(/^file:\/\//, "") : "",
						).onChange((value) => {
							this.uris[i] = "file://" + value;
						});
					} else {
						text.setValue(this.uris[i] || "").onChange((value) => {
							this.uris[i] = value;
						});
					}
	...
	submit() {
		...
		if (!this.folderPath) {
			new Notice(
				`${this.plugin.manifest.name}: The folder to save the dummy files is not specified.`,
			);
			return;
		}
	...
	async createDummyFiles() {
		if (this.folderPath) {
			...
			const files = await this.lib.dummyFileManager.createDummyFilesInFolder(
				this.folderPath,
				this.uris,
			);
			...
			new Notice(
				`${this.plugin.manifest.name}: Failed to create dummy files for the following URIs: ${this.uris.join(", ")}`,
			);
		}
	}
}
