 ...
 * @param quadPoints
 */
export function pdfJsQuadPointsToArrayOfRects(
	quadPoints: Float32Array | Array<Array<{ x: number; y: number }>>,
): Array<Rect> {
	const rects: Rect[] = [];
...
export function showMenuUnderParentEl(menu: Menu, parentEl: HTMLElement) {
	...
	menu.setParentElement(parentEl).showAtPosition({
		x,
		y: bottom,
		width,
		overlap: true,
...
export function getOffsetInTextLayerNode(
	...
	if (node.nodeType === Node.ELEMENT_NODE) {
		// Offset refers to the child node index
		let offset = 0;
		...
		// Then add the offset within the parent by recursively calling for the parent
		return getOffsetInTextLayerNode(textLayerNode, node.parentNode!, 0)! + offset;
	}
	...
	while ((textNode = iterator.nextNode()) && node !== textNode) {
		// Iterate over text nodes that come before `node`.
...
export function* toPDFCoords(
	...
	screenCoords: Iterable<{ x: number; y: number }>,
) {
...
export function* getCharactersWithBoundingBoxesInPDFCoords(
	pageView: PDFPageView,
	textLayerNode: HTMLElement,
) {
...
export function getTextLayerInfo(
	textLayerBuilder: TextLayerBuilder | OldTextLayerBuilder,
): { textDivs: HTMLElement[]; textContentItems: TextContentItem[] } | null {
	if ("textLayer" in textLayerBuilder) {
		// true if Obsidian is v1.8.0 or later
...
export function getAndroidWebViewVersion() {
	...
	if ("userAgentData" in navigator && navigator.userAgentData) {
		const androidWebViewBrand = navigator.userAgentData.brands.find(
			(brand) => brand.brand === "Android WebView",
		);
...
export async function getObsidianDebugInfo(app: App) {
	...
	const numSnippets = app.customCss.snippets.filter((e) =>
		app.customCss.enabledSnippets.has(e),
	).length;
...
export function findReferenceCache(
	...
	return (
		cache.links?.find(
			(link) => start <= link.position.start.offset && link.position.end.offset <= end,
		) ??
		cache.embeds?.find(
			(embed) => start <= embed.position.start.offset && embed.position.end.offset <= end,
		)
	);
}
...
export function parsePDFSubpath(
	subpath: string,
):
...
export function isAncestorOf<TreeNode extends { children: TreeNode[]; parent: TreeNode | null }>(
	one: TreeNode,
	another: TreeNode,
	includeSelf = false,
): boolean {
...
export function toSingleLine(str: string, removeWhitespaceBetweenCJChars = false): string {
	...
	// https://github.com/RyotaUshio/obsidian-pdf-reader/issues/173
...
export function encodeLinktext(linktext: string) {
	...
	return linktext.replace(/[\\\x00\x08\x0B\x0C\x0E-\x1F ]/g, (component) =>
...
export function binarySearch<Item>(
	array: Item[],
	cmp: (item: Item, index: number) => number,
	options?: Partial<{ from: number; to: number; findFirst: boolean; findLast: boolean }>,
...
export function binarySearchForRangeStartingWith<T>(
	array: T[],
	prefix: string,
	getItemText: (item: T) => string,
	options?: Partial<{ from: number; to: number }>,
) {
...
export function areOverlappingStrictly(
	range1: { from: number; to: number },
	range2: { from: number; to: number },
) {
	return range1.from < range2.to && range1.to > range2.from;
 ...
 */
export function walkDescendantComponents(
	component: Component,
	callback: (component: Component) => boolean | void,
) {
